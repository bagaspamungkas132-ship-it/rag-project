...
# Feature engineering ON -- bedanya dari run sebelumnya: sekarang fitur hasil
# engineering ikut masuk ke proses cleaning otomatis yang sama (redundancy
# clustering + permutation importance). Jadi kalau fitur hasil rasio ternyata
# cuma "menduplikasi" sinyal fitur mentahnya, otomatis kebuang di Step 2/5 --
# tidak perlu lagi nebak manual mana yang genuinely berguna.
...
# %% ------------------------------------------------------------------
# 1b. FEATURE ENGINEERING (opsional, lihat USE_ENGINEERED_FEATURES di CONFIG)
#     Interaksi/rasio antar fitur -- akan diuji objektif lewat cluster redundancy
#     (Step 2) & permutation importance (Step 5), bukan diputuskan manual.
# ------------------------------------------------------------------
...
if USE_ENGINEERED_FEATURES:
    df_fds = add_engineered_features(df_fds)
    print(f"Feature engineering ON -- fitur baru: {[c for c in df_fds.columns if c.startswith('fe_')]}")
else:
    print("Feature engineering OFF")
