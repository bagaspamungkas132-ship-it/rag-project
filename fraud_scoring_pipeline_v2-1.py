...
# --- Pilih model terbaik otomatis, berdasarkan ROC-AUC tertinggi di test set ---
proba_by_model = {"LogReg": proba_lr, "RandomForest (tuned)": proba_rf, "XGBoost (tuned)": proba_xgb}
best_row = results_summary.loc[results_summary["roc_auc"].idxmax()]
best_model_name = best_row["model"]
best_proba = proba_by_model[best_model_name]

print(f"\n>> MODEL TERBAIK: {best_model_name} "
      f"(ROC-AUC={best_row['roc_auc']:.3f}, PR-AUC={best_row['pr_auc']:.3f})")
if (results_summary["roc_auc"].max() - results_summary["roc_auc"].min()) < 0.02:
