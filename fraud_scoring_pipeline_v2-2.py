...
# Feature engineering (rasio/interaksi) terbukti bikin importance ambigu -- karena
# rasio itu turunan langsung dari fitur mentahnya (misal fe_out10x_per_account_age
# = n_out_10x_income / account_age), jadi tree model "berebut" sinyal antara
# versi mentah vs versi rasio. Default sekarang OFF. Set True kalau mau bandingkan.
USE_ENGINEERED_FEATURES = False

# Tuning agresif (n_iter besar) terbukti overfit ke CV fold di data sekecil ini
# (Best RF CV ROC-AUC 0.723 vs test cuma 0.652). Turunkan supaya lebih konservatif.
TUNING_N_ITER = 10

# Setelah lihat laporan "pasangan fitur redundant" di Step 3, isi list di bawah ini
# dengan nama kolom yang kamu putuskan mau dibuang, lalu run ulang.
# Contoh: MANUAL_DROP_FEATURES = ["n_sameday_pass_1h_l3m", "transfer_not_normal_l6m_pct"]
...
# %% ------------------------------------------------------------------
# 3. INFO FITUR REDUNDANT (TIDAK di-drop otomatis)
#    Ini cuma laporan buat kamu putuskan sendiri fitur mana yang mau dibuang.
#    Isi MANUAL_DROP_FEATURES di bagian CONFIG paling atas kalau sudah yakin.
#
#    Catatan analis: kalau dua fitur redundant secara matematis tapi punya makna
#    bisnis beda (misal "count transaksi 3 jam" vs "1 jam" -> beda kecepatan alert),
#    kadang lebih baik dipertahankan meski korelasinya tinggi -- keputusan ini
#    idealnya bukan cuma statistik, tapi juga masukan tim fraud.
# ------------------------------------------------------------------
    ...
    for j in range(i + 1, len(cols_m)):
        ...
        if pd.notna(v) and abs(v) > 0.85:
            redundant_pairs.append({
                "feature_a": a, "feature_b": b, "corr_ab": round(v, 3),
                "corr_a_to_target": round(corr_to_target.get(a, 0), 3),
                "corr_b_to_target": round(corr_to_target.get(b, 0), 3),
                "saran_buang": b if corr_to_target.get(a, 0) >= corr_to_target.get(b, 0) else a,
            })

redundant_df = pd.DataFrame(redundant_pairs)
print("=== Pasangan fitur redundant (|corr| > 0.85) -- info saja, TIDAK di-drop otomatis ===")
print(redundant_df.to_string(index=False) if len(redundant_df) else "(tidak ada)")

feature_cols_clean = [c for c in feature_cols if c not in MANUAL_DROP_FEATURES]
if MANUAL_DROP_FEATURES:
    print(f"\nFitur yang kamu drop manual via MANUAL_DROP_FEATURES: {MANUAL_DROP_FEATURES}")
print(f"Fitur dipakai: {len(feature_cols_clean)} dari {len(feature_cols)}")

# --- Cek fitur near-constant (biasa jadi penyebab importance=0 di tree model,
#     walau korelasinya kelihatan signifikan secara statistik) ---
print("\n=== Cek distribusi fitur binary/flag (deteksi near-constant) ===")
flag_like_cols = [c for c in feature_cols_clean if df_fds[c].dropna().nunique() <= 3]
for c in flag_like_cols:
    vc = df_fds[c].value_counts(normalize=True, dropna=False)
    dominant_pct = vc.iloc[0]
    flag_note = " << DOMINAN >95%, kemungkinan ini kenapa importance-nya 0 di tree model" if dominant_pct > 0.95 else ""
    print(f"  {c}: {dict(vc.round(3))}{flag_note}")
...
df_fds_fe = add_engineered_features(df_fds, feature_cols_clean) if USE_ENGINEERED_FEATURES else df_fds.copy()
new_fe_cols = [c for c in df_fds_fe.columns if c.startswith("fe_")] if USE_ENGINEERED_FEATURES else []
feature_cols_final = feature_cols_clean + new_fe_cols
if USE_ENGINEERED_FEATURES:
    print("Fitur baru hasil engineering:", new_fe_cols)
else:
    print("USE_ENGINEERED_FEATURES=False -- pakai fitur mentah saja (rekomendasi saat ini, "
          "lihat catatan di CONFIG kenapa)")
...
# --- Cek overlap accountid train vs test (poin risiko leakage dari akun berulang) ---
train_accounts = set(train_df["accountid"])
test_accounts = set(test_df["accountid"])
overlap_accounts = train_accounts & test_accounts
overlap_rows_test = test_df["accountid"].isin(train_accounts).sum()
print(f"\nAkun unik di train: {len(train_accounts)} | di test: {len(test_accounts)}")
print(f"Akun yang overlap (muncul di train DAN test): {len(overlap_accounts)} "
      f"({len(overlap_accounts) / max(len(test_accounts), 1):.1%} dari akun test)")
print(f"Baris di test yang akunnya sudah 'dikenal' model dari train: {overlap_rows_test} "
      f"({overlap_rows_test / len(test_df):.1%} dari test)")
if overlap_rows_test / len(test_df) > 0.15:
    print(">> PERHATIAN: overlap akun cukup tinggi. Performa test bisa sedikit ke-boost "
          "karena model 'mengenal' sebagian akun ini dari data train, bukan murni generalisasi.")
...
#    n_iter sengaja dikecilkan (lihat TUNING_N_ITER di CONFIG) + min_samples_leaf/
#    max_depth dibatasi lebih konservatif -- data cuma ratusan baris, model kompleks
#    gampang overfit ke fold CV (lihat catatan gap CV-vs-test di bawah).
# ------------------------------------------------------------------
tscv = TimeSeriesSplit(n_splits=3)

rf_param_dist = {
    "n_estimators": [200, 300, 500],
    "max_depth": [3, 4, 5, 6],          # dibatasi, jangan sampai None (bisa overfit total)
    "min_samples_leaf": [15, 20, 30, 40],  # dinaikkan, biar tiap leaf punya cukup sampel
    "max_features": ["sqrt", 0.5, 0.8],
}
rf_search = RandomizedSearchCV(
    RandomForestClassifier(class_weight="balanced_subsample", random_state=RANDOM_STATE, n_jobs=-1),
    param_distributions=rf_param_dist, n_iter=TUNING_N_ITER, scoring="roc_auc",
...
xgb_param_dist = {
    ...
    "max_depth": [2, 3, 4],             # dibatasi lebih dangkal
    "learning_rate": [0.02, 0.05, 0.08],
    "subsample": [0.6, 0.8],
    "colsample_bytree": [0.6, 0.8],
    "min_child_weight": [5, 10, 15],    # dinaikkan, lebih konservatif
    "reg_lambda": [1, 3, 5, 10],        # regularisasi L2 tambahan
}
xgb_search = RandomizedSearchCV(
    xgb.XGBClassifier(scale_pos_weight=scale_pos_weight, eval_metric="auc", random_state=RANDOM_STATE),
    param_distributions=xgb_param_dist, n_iter=TUNING_N_ITER, scoring="roc_auc",
    cv=tscv, random_state=RANDOM_STATE, n_jobs=-1, verbose=0,
)
...
# %% ------------------------------------------------------------------
# 7. LOGISTIC REGRESSION -- juga di-tuning (cari C/regularisasi terbaik),
#    karena di run sebelumnya LogReg default justru mengalahkan RF & XGB yang di-tuning
# ------------------------------------------------------------------
...
lr_search = RandomizedSearchCV(
    ...
    param_distributions=lr_param_dist, n_iter=6, scoring="roc_auc",
    cv=tscv, random_state=RANDOM_STATE, n_jobs=-1, verbose=0,
)
lr_search.fit(X_train_scaled, y_train)
print("Best LogReg params:", lr_search.best_params_)
print("Best LogReg CV ROC-AUC:", lr_search.best_score_)
...
results_summary = pd.DataFrame({
    ...
    "cv_roc_auc": [lr_search.best_score_, rf_search.best_score_, xgb_search.best_score_],
    "test_roc_auc": [roc_auc_score(y_test, proba_lr), roc_auc_score(y_test, proba_rf), roc_auc_score(y_test, proba_xgb)],
...
results_summary["cv_test_gap"] = results_summary["cv_roc_auc"] - results_summary["test_roc_auc"]
print(results_summary.to_string(index=False))

for _, row in results_summary.iterrows():
    if row["cv_test_gap"] > 0.05:
        print(f">> PERHATIAN: {row['model']} -- gap CV vs test = {row['cv_test_gap']:.3f} (CV jauh lebih "
              f"tinggi dari test). Indikasi model ini overfit ke fold CV, jangan terlalu percaya "
              f"skor CV-nya sebagai estimasi performa asli.")

# --- Pilih model terbaik otomatis, berdasarkan ROC-AUC tertinggi di test set (bukan CV) ---
proba_by_model = {"LogReg": proba_lr, "RandomForest (tuned)": proba_rf, "XGBoost (tuned)": proba_xgb}
best_row = results_summary.loc[results_summary["test_roc_auc"].idxmax()]
best_model_name = best_row["model"]
best_proba = proba_by_model[best_model_name]

print(f"\n>> MODEL TERBAIK: {best_model_name} "
      f"(test ROC-AUC={best_row['test_roc_auc']:.3f}, PR-AUC={best_row['pr_auc']:.3f})")
if (results_summary["test_roc_auc"].max() - results_summary["test_roc_auc"].min()) < 0.02:
    print(">> Catatan: selisih ROC-AUC antar model tipis (<0.02) -- dengan data sekecil ini, "
          "perbedaan ini bisa jadi cuma noise, bukan model yang benar-benar lebih unggul. "
          "Pertimbangkan juga interpretability (LogReg lebih mudah dijelaskan ke auditor/regulator)")
