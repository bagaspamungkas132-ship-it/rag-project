...
def id_status(ktp_norm, npwp_norm, other_id_norm):
    ...
    MATCH -> ktp or npwp equals the candidate id (shouldn't normally reach
             ...
             kept for safety)
    DIFFERENT -> trx has a ktp/npwp value but it doesn't equal the candidate id
    """
...
def match_screening(trx, screening):
    ...
    if needs_fuzzy:
        ...
        score_matrix_alias = process.cdist(
            query_names, alias_choices, scorer=fuzz.token_sort_ratio, dtype=np.uint8, workers=-1
        )
        score_matrix_name = process.cdist(
            query_names, name_choices, scorer=fuzz.token_sort_ratio, dtype=np.uint8, workers=-1
        )
        # take whichever field (alias vs name) gives the better score
        combined = np.maximum(score_matrix_alias, score_matrix_name)
        from_alias = score_matrix_alias >= score_matrix_name

        best_pos = combined.argmax(axis=1)
        best_scores = combined[np.arange(len(needs_fuzzy)), best_pos]
