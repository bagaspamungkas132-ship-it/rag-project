...
Requirements:
    pip install pandas pyarrow rapidfuzz openpyxl --break-system-packages
...
Output (each result is written as BOTH a new parquet file and an .xlsx file -
the original trx_all/dpn/screening parquet files are never overwritten):
  - trx_all_enriched.parquet / .xlsx: original trx_all columns (same names,
    order, and data types as the source parquet - numbers stay numbers,
    dates stay dates) + dpn_* columns (from dpn matching) + screening_*
    columns (from screening matching, including screening_comment and every
    screening column checked, when a match is found), plus dpn_flag and
    screening_flag (1/0). Both sets of match columns are blank when there's
    no match for that table.
  - customer_match_result.parquet / .xlsx: a compact match-only summary
    covering both dpn and screening matches (with the same dpn_flag /
    screening_flag columns), useful for QA.

  dpn_flag / screening_flag = 1 when that table's match_type is one of
  PERFECT_MATCH, ID_MATCH, NAME_DOB_MATCH, or NAME_DOB_SIMILAR - i.e. a
  "real" match - and 0 otherwise (NAME_ONLY_MATCH_ID_EMPTY,
  NAME_ONLY_MATCH_ID_DIFFERENT, NO_MATCH).

  The .xlsx files are plain data exports (no formulas) with a bold header
  row, frozen header, and auto-sized columns for readability.
"""
...
# New files only - the original trx_all/dpn/screening parquet files are never
# overwritten.
ENRICHED_PARQUET_PATH = "/home/cdsw/parquet_output/trx_all_enriched.parquet"
ENRICHED_XLSX_PATH = "/home/cdsw/parquet_output/trx_all_enriched.xlsx"
SUMMARY_PARQUET_PATH = "/home/cdsw/parquet_output/customer_match_result.parquet"
SUMMARY_XLSX_PATH = "/home/cdsw/parquet_output/customer_match_result.xlsx"

# match_type categories that count as a "positive" match for the 1/0 flag
FLAG_MATCH_CATEGORIES = {"PERFECT_MATCH", "ID_MATCH", "NAME_DOB_MATCH", "NAME_DOB_SIMILAR"}
...
# Load & prepare
# ---------------------------------------------------------------------------
def match_flag(match_type_array):
    """1 if match_type is PERFECT_MATCH / ID_MATCH / NAME_DOB_MATCH /
    NAME_DOB_SIMILAR, else 0."""
    return np.array(
        [1 if mt in FLAG_MATCH_CATEGORIES else 0 for mt in match_type_array], dtype=int
    )
...
def match_customers(trx, dpn):
    ...
    for i, row in trx.iterrows():
        d_idx = dpn_by_id.get(row["_ktp_norm"])
        if d_idx is None:
            d_idx = dpn_by_id.get(row["_npwp_norm"])
...
def match_screening(trx, screening):
    ...
    needs_fuzzy = []
    for i, row in trx.iterrows():
        s_idx = screening_by_id.get(row["_ktp_norm"])
        if s_idx is None:
            s_idx = screening_by_id.get(row["_npwp_norm"])
...
def build_enriched_trx(
    ...
    enriched["dpn_flag"] = match_flag(dpn_match_type)
    ...
    enriched["screening_flag"] = match_flag(scr_match_type)
...
def build_summary(
    ...
    dpn_flags = match_flag(dpn_match_type)
    scr_flags = match_flag(scr_match_type)
...
def save_excel(df, path, sheet_name="Sheet1"):
    """Write df to an .xlsx file, keeping the same columns/values/dtypes as
    the source data (numbers stay numeric, dates stay dates - pandas'
    to_excel preserves this automatically), with light professional
    formatting: bold header, frozen header row, auto-sized columns.
    """
...
def main():
    ...
    # Parquet: same columns/dtypes as the original trx_all data, just with the
    # dpn_*/screening_*/flag columns appended - written as a brand new file,
    # the original trx_all parquet is never touched.
    enriched.to_parquet(ENRICHED_PARQUET_PATH, index=False)
    print(f"\nSaved enriched trx_all (parquet) to {ENRICHED_PARQUET_PATH}")
    save_excel(enriched, ENRICHED_XLSX_PATH, sheet_name="trx_all_enriched")
    print(f"Saved enriched trx_all (excel) to {ENRICHED_XLSX_PATH}")
    ...
    print(f"Saved match summary (parquet) to {SUMMARY_PARQUET_PATH}")
    save_excel(summary, SUMMARY_XLSX_PATH, sheet_name="match_summary")
    print(f"Saved match summary (excel) to {SUMMARY_XLSX_PATH}")
